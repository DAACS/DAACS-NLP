...
def main():
    ...
    model_args = ClassificationArgs(num_train_epochs=3, output_dir=b.SIMPLE_DIR, overwrite_output_dir=True)

    if os.path.exists(b.SIMPLE_DIR) and os.listdir(b.SIMPLE_DIR):
        print("Loading the existing model...")
        model = ClassificationModel("distilbert",b.SIMPLE_DIR, args=model_args, use_cuda=False)
    else:
        print("Training a new model...")
        model = ClassificationModel("distilbert", "distilbert-base-uncased", 
                                    num_labels=num_unique_labels, args=model_args, use_cuda=False)
